...
document.getElementById('validateButton').addEventListener('click', event => {
    ...
    document.querySelectorAll('input').forEach(element => {
        if (element.value) return alertMessage.push(checkUserInputValue(checkValueItem[element.id].item,element,checkValueItem[element.id].pattern));
...
// 共通入力チェック、パターンチェック関数
function checkUserInputValue(item, element, pattern) {
    if (element.value.match(pattern)) console.log(`${item}が正しく入力されました。:(${element.value})`)
    else return `${item}を正しく入力し直してください。:(${element.value})`
}
